...
class Network(object):
    ...
    def train_nobias(self, features, labels, epochs, N): 
        for epoch in range(epochs):
            loss = 0.0
            for x, y in zip(features, labels):
                E = (np.dot(self.w_nobias,x) * y)[0]
                E_ = (E <= self.c).astype(float)
                self.w_nobias += (1.0 / N) * E_ * x * y
                loss += E_
            self.error.append(loss)
            if loss <= 0:
                break        
        ...
        for idx_n, N in enumerate(tqdm([20, 60, 100])):
            ...
            for idx_alpha, alpha in enumerate(tqdm(alphas)):
                ...
                for run in range(nd):
                    ...
                    labels = (np.random.randint(0,2,[P, 1])*2)-1
                    model = Network(N)
                    success.append(model.train_nobias(features, labels, epochs, N))
                runs[idx_alpha] = np.mean(success)
